...
class MagicInfoFallbackClient(QualityClient):
    def _get_card_by_printing(self, set_code, collector_number, language):
        if language == "es":
            return card("es", set_code, collector_number, "lowres", False)
        return None
    ...
    def _find_magiccardsinfo_fallback(self, card, selected):
        ...
        upgraded.update(
            {
                ...
                "highres_image": True,
                "image_uris": {
                    "png": "https://magiccards.info/scans/es/test.png",
                },
            }
        )
        return upgraded
...
def test_magiccardsinfo_replaces_spanish_lowres() -> None:
    client = MagicInfoFallbackClient()
    try:
        result = client.resolve(
            DeckCard(1, "Arcane Signet", set_code="tmc", collector_number="57"),
            resolution_mode="exact_first",
    ...
    assert result.provider == "magiccardsinfo"
    assert result.status == "Misma impresión en español · MagicCards.info"
    assert result.highres_image is True
    assert result.faces[0].provider == "magiccardsinfo"


def test_highres_only_accepts_magiccardsinfo_fallback() -> None:
    client = MagicInfoFallbackClient()
    try:
        result = client.resolve(
            DeckCard(1, "Arcane Signet", set_code="tmc", collector_number="57"),
